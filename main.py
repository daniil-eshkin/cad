...
from cad.util import calibrate_box
...
def main(conf_path, model_path, input_path):
    ...
    # Match bounding box and grid and generate grid points
    box = calibrate_box(box, conf)
    x = np.arange(box.min_x, box.max_x + 1, conf['grid']['length'])
    y = np.arange(box.min_y, box.max_y + 1, conf['grid']['length'])
    z = np.arange(box.min_z - conf['grid']['length'], box.max_z + 1, conf['grid']['length'])
    ...
    # Visualize
    show_model(floors, objs, grid.points, start, end, path, conf)
